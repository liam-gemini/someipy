...
class ClientServiceInstance(ServiceDiscoveryObserver):
    ...
    async def call_method_fire_forget(
        self, method_id: int, payload: bytes
    ) -> Tuple[MethodResult, bytes]:
    ...
    def someip_message_received(
        ...
        if (
            #someip_message.header.client_id == 0x00 and 
            someip_message.header.message_type == MessageType.NOTIFICATION.value
        ...
        if (
            #someip_message.header.client_id == 0x00 and
            someip_message.header.message_type == MessageType.REQUEST_NO_RETURN.value
            and someip_message.header.return_code == 0x00
        ):
            if self._callback is not None:
                self._callback(someip_message)
